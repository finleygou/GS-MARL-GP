...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world: World) -> None:
        ...
        agents_added = []
        while True:
            if num_agents_added == self.num_egos:
                break
            random_pos = np.random.uniform(
                -self.world_size / 2, self.world_size / 2, world.dim_p
            )
            agent_size = world.egos[num_agents_added].size
            ...
            if not obs_collision and not agent_collision:
                world.egos[num_agents_added].state.p_pos = random_pos
                world.egos[num_agents_added].state.p_vel = np.zeros(world.dim_p)
                agents_added.append(world.egos[num_agents_added])
                num_agents_added += 1

        num_targets_added = 0
        targets_added = []
        while True:
            if num_targets_added == self.num_egos:
                break
            random_pos = 0.8 * np.random.uniform(
                -self.world_size / 2, self.world_size / 2, world.dim_p
            )
            target_size = world.targets[num_targets_added].size
            obs_collision = self.is_obstacle_collision(random_pos, target_size, world)
            target_collision = self.is_target_collision(
                random_pos, target_size, world.targets[:num_targets_added]
            )
        ...
        #     print(f"Agent {i} position: {world.egos[i].state.p_pos}")
        #     print(f"Obstacle {i} position: {world.obstacles[i].state.p_pos}")
        #     print(f"Target {i} position: {world.targets[i].state.p_pos}")
        '''
    ...
    def _get_entity_feat_relative(self, agent: Agent, entity: Entity, world: World) -> arr:
        ...
        return np.hstack([pos, vel, Radius, entity_type])  # dim = 5
